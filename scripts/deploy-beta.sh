...
NAMESPACE="${NAMESPACE:-cerberus-beta}"
...
build_and_push_images() {
    ...
    for svc in "${SERVICES[@]}"; do
        ...
        docker build -t "$IMAGE_REGISTRY/cerberus-$svc:$IMAGE_TAG" -t "$IMAGE_REGISTRY/cerberus-$svc:beta" -f "$PROJECT_ROOT/services/$svc/Dockerfile" "$PROJECT_ROOT"
        docker push "$IMAGE_REGISTRY/cerberus-$svc:$IMAGE_TAG"
        docker push "$IMAGE_REGISTRY/cerberus-$svc:beta"
...
main() {
    log_section "Cerberus - Beta Deployment"
